...
from nandi.GroupSet.functions.handlers import CMD_STARTERS
from nandi.GroupSet.functions.misc import is_module_loaded

FILENAME = __name__.rsplit(".", 1)[-1]

# If module is due to be loaded, then setup all the magical handlers
if is_module_loaded(FILENAME):
    from nandi.GroupSet.functions.chat_status import user_admin, is_user_admin
    from telegram.ext.dispatcher import run_async

    from nandi.GroupSet.sql import disable_sql as sql
    ...
    class DisableAbleCommandHandler(CommandHandler):
        #handler class to use /disable command in the group
    ...
    def disable(bot: Bot, update: Update, args: List[str]):
        #/disable method to disable a command. commands can be given followed by /disable command.
        chat = update.effective_chat  # type: Optional[Chat]
        if len(args) >= 1:
            #if the length of arg is greater than one.
            disable_cmd = args[0]
    ...
    def enable(bot: Bot, update: Update, args: List[str]):
        #/enable method to enable a command. commands can be given followed by /enable command.
        chat = update.effective_chat  # type: Optional[Chat]
        if len(args) >= 1:
            #if the length of arg is greater than one.
            enable_cmd = args[0]
    ...
    def list_cmds(bot: Bot, update: Update):
        #cmds to show list cmds, that can be disabled in a group chat.
        if DISABLE_CMDS + DISABLE_OTHER:
            result = ""
            for cmd in set(DISABLE_CMDS + DISABLE_OTHER):
                #loading list of cmds.
